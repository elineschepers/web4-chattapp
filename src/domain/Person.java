...
public class Person {
	...
	public void addVriend(Person p)
	{
		if(vrienden.size()!=0)
		{
			for(Person person : vrienden)
			{
				if(person.getUserId().equals(p.getUserId()))
				{
					System.out.println("vriend is al reeds toegevoegd");
					return;
				}
			}
			vrienden.add(p);
	...
	public ArrayList<Person> getVrienden(){return vrienden;}
